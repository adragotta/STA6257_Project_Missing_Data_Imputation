...
Upon viewing visualizations of the missing data distribution. One can
see certain fields contain the most missing values. These fields such as
body, cabin, boat, age and home.dest. One can also observe from the
plots from the titantic dataset that 79% of the data is present and 21%
is missing. The missing variables percentage plot displays the
percentage for the certain variables. fields with the most missing data
belonging to the body variable. One could consider that the variables
such as body may be influenced by another factor when hypothesizing what
kind of missing data type each of these fields belong to.
