...
at random. Regarding missing completely at random (MCAR) data, all of
the data points have the same odds to be missing. Missing at random is
attributable to things like the patient’s condition, which means the
missingness of the data is independent of the value of the data. Not
missing random data completely depends on the value of the data. For
each of these categories, the level of bias can be different
[@jazayeri2020imputation].
...
The code creates a plot that displays if there is a missing element in
the dataset's columns. Grey indicating that there is a missing element.
...
Upon viewing the visualizations of the missing data distribution for the
titanic data set. One can see certain fields contain the most missing
values. These fields such as body, cabin, boat, age and home.dest. The
missing variables percentage plot displays these percentages for the
certain variables. One can also observe from the titatanic data set
plots that overall 79% of the data is present and 21% is missing.
Specifically, the body variable contains data which 91% is missing,
followed by cabin 77% and boat 63%. One could consider that the
variables such as body may be influenced by another factor when
hypothesizing what kind of missing data type each of these fields belong
to and what imputation methods one could apply.
...
the Titanic dataset. White indicates missing.
